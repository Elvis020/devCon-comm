...
|---|---|---|---|---|
| P0 | Organizer auth | Organizer access uses one shared password and a cookie value derived from `ADMIN_SESSION_SECRET`; there are no per-admin identities, roles, lockouts, or audit trails. | `server/app.ts` `adminPassword`, `adminSessionToken`, `requireAdmin`, and `/api/auth/admin/login` | Replace with Supabase Auth or another identity provider, add role checks in Hono middleware, and audit organizer mutations. |
